...
class _SearchModuleState extends State<SearchModule> {
                  ...
                  children: [
                        ...
                        onTap: () {
                          if (currentSearchIndex != 0) {
                            fetchResults(constraints,
                                searchData:
                                    searchDataList[currentSearchIndex - 1]);
                          }
                        },
                        child: Icon(Icons.keyboard_arrow_left,
                            color: currentSearchIndex == 0 ? text4 : text2,
                            size: 42)),
                    ...
                    InkWell(
                        ...
                        child: Icon(Icons.keyboard_arrow_right,
                            color:
                                currentSearchIndex < searchDataList.length - 1
                                    ? text2
                                    : text4,
                            size: 42)),
                    SizedBox(width: constraints.maxWidth * 0.01),
                    searchController.text.isEmpty
                        ? Icon(Icons.search, color: text2, size: 32)
                        : loading
                            ? SizedBox(
                                height: 25,
                                width: 25,
  ...
  Future<void> fetchResults(BoxConstraints constraints,
      {int pageNumber = 0, SearchData? searchData}) async {
    loading = true;
    if (searchData != null) {
      currentSearchIndex = searchDataList.indexOf(searchData);
      searchController.text = searchData.query;
      switch (searchData.typeOfSearch) {
        case 'Images':
          await showImageResults(constraints);
          return;
      }
    } else {
      currentSearchIndex = searchDataList.length;
      searchDataList
          .add(SearchData(query: searchController.text, typeOfSearch: 'All'));
    }
    selectedTypesOfResults = 'All';
    String query =
        searchData == null ? searchController.text : searchData.query;
    setState(() {});
    numberOfTabs++;
    Uri url = Uri.parse(
        'https://serpapi.com/search?start=${(pageNumber * 10).toString()}&num=10&q=$query&engine=google&api_key=${serpApiKey}');
    ...
    if (webPage['related_searches'] != null) {
      print(webPage['related_searches']);
  ...
  Future<void> showImageResults(BoxConstraints constraints) async {
    loading = true;
          ...
          Container(
            ...
            child: GridView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return InkWell(
                    onTap: () {
                      widget.onTap(relatedSearches[index]['query']);
                    },
                    child: Container(
...
class SearchData {
  ...
  SearchData({required this.query, required this.typeOfSearch});
}
